    ...
    <div className="page-container">
      ...
      <div className="product-grid">
        {products.map((product) => {
          ...
          return (
          <div key={product.id} className="card glass-panel">
            <img
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
              {selectedProductForSize.sizes.map(size => (
                <button
                  ...
                  style={{
                    padding: '1rem',
                    border: selectedSize?.id === size.id ? '2px solid var(--accent)' : '1px solid #d1d5db',
                    borderRadius: '8px',
                    background: selectedSize?.id === size.id ? 'var(--accent)' : '#fff',
                    color: selectedSize?.id === size.id ? '#fff' : 'inherit',
                    cursor: 'pointer',
                    fontWeight: '500',
                    transition: 'all 0.2s'
                  }}
